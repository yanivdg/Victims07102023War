...
export class HomeComponent implements OnInit{
  ...
  codeTranslateDictionary: Record<string, any> = {
    ...
    r: {combo:'Rescue',image:
    "url('https://upload.wikimedia.org/wikipedia/commons/thumb/3/36/Red_Star_of_David.svg/1280px-Red_Star_of_David.svg.png'),url('https://upload.wikimedia.org/wikipedia/he/thumb/d/d6/FireDepIsraelOld.svg/1024px-FireDepIsraelOld.svg.png')"}
  };
...
convertToTable(htmlString: string): string {
  ...
  addPropertyToElement(htmlString: string,PropertyName:string,PropertyValue:string): string {
    ...
    elements.forEach((element: any) => {
      ...
      content.style.backgroundBlendMode = 'multiply'; /* Blend mode to merge images (you can experiment with different blend modes) */
    });
...
/*
  this.dataService.postRequest(jsonbody).pipe(
    switchMap((response:any) => {
      return new Observable((observer) => {
        observer.next(response); // Pass the response to the subscriber
        observer.complete(); // Complete the Observable
